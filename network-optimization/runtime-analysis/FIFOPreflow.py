...
def reverse_bfs(FStar, s):
    ...
    A = AI_list(FStar)
    # Number of nodes
    n = len(np.unique(FStar[:,[0,1]].reshape(-1,)))
    # Tails of arcs
    T = FStar[:,0]
    H = FStar[:,1]
    # Heads of arcs
...
def push_or_relabel(RFStar, A, AO, AI, e, d, i, t):
    ...
    '''
    n = len(np.unique(RFStar[:,[0,1]].reshape(-1,)))
    j = None
    relabeled = False
...
def FIFOPreflow(FStar, s, t, output=True):
    """
    ...
    if output:
        ...
        RFStar = RFStar[idx, :]
        print "FIFOPreflow push algorithm"
        print "=========================="
    ...
    return x, max_flow, True
if __name__ == "__main__":
    s = 1
    FStar = np.loadtxt("FStard.txt", )
    t = np.amax(np.unique(FStar[:,0:2].reshape(-1,)))
    x , max_flow, z = FIFOPreflow(FStar, s, t)
